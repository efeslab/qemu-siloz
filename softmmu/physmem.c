...
static void ram_block_add(RAMBlock *new_block, Error **errp)
{
    ...
    if (new_block->host && memory_access_is_direct(new_block->mr, 0)) {
        if (strcmp(new_block->mr->name, "pc.ram") == 0) {
            if (got_pc_ram) {
                printf("NAME: %s\n", new_block->mr->name);
                assert(false);
            } else {
    ...
    if (!new_block->host) {
        if (xen_enabled()) {
            ...
            new_block->host = qemu_anon_ram_alloc(new_block->max_length,
                                                  &new_block->mr->align,
                                                  shared, noreserve, memory_access_is_direct(new_block->mr, 0));
            if (!new_block->host) {
                error_setg_errno(errp, errno,

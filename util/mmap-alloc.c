...
#define MAP_GUEST		  0x8000000

/*
 * Reserve a new memory region of the requested size to be used for mapping
 * from the given fd (if any).
 */
static void *mmap_reserve(size_t size, int fd, bool is_direct)
{
    ...
    return mmap(0, size, PROT_NONE, flags | (is_direct ? MAP_GUEST : 0), fd, 0);
}
...
static void *mmap_activate(void *ptr, size_t size, int fd,
                           uint32_t qemu_map_flags, off_t map_offset, bool is_direct)
{
    ...
    activated_ptr = mmap(ptr, size, prot, flags | map_sync_flags | (is_direct ? MAP_GUEST : 0), fd,
                          map_offset);
    if (activated_ptr == MAP_FAILED && map_sync_flags) {
         ...
         */
        activated_ptr = mmap(ptr, size, prot, flags | (is_direct ? MAP_GUEST : 0), fd, map_offset);
    }
                    ...
                    off_t map_offset)
{
    ...
    guardptr = mmap_reserve(total, fd, qemu_map_flags & QEMU_MAP_GUEST);
    ...
    ptr = mmap_activate(guardptr + offset, size, fd, qemu_map_flags,
                        map_offset, qemu_map_flags & QEMU_MAP_GUEST);
    if (ptr == MAP_FAILED) {
